"""Experiment 3 — Measurement-noise robustness sweep (no Kalman filter).

WHAT IT DOES
    Sweeps Gaussian sensor noise injected on every follower's per-edge
    relative-position measurement, with the formation cruising at a common
    base_vel = [0.5, 0, 0] m/s. Locates the engineering robustness boundary:
    the formation stays usable up to a threshold sigma, then degrades and
    eventually goes unstable. Run Experiment 4 (KF) to see this boundary
    pushed outward.

    Noise model (per follower i, per neighbor j, independent each step):
        z_ij_meas = R_z(-yaw_i) (x_j - x_i) + n_ij,   n_ij ~ N(0, sigma^2 I_3)
        u_i^local = sum_j M_ij @ z_ij_meas
        u_i^world = R_z(+yaw_i) u_i^local
    Leaders (UAV_L, UGV) have zero formation feedback. The clean HIL sigma=0
    run is used as the comparison baseline (no ODE comparison here).

HOW TO RUN
    python -m hetero_cosim.experiments.noise
    Each sigma's HIL trajectory is cached in hetero_noise_moving/raw/.
    Delete that folder to force a fresh sweep.

CONFIG (edit constants near the top)
    SIGMAS   list of noise std values to sweep (meters).
    PICK     the three sigmas drawn on the figures (weak/medium/strong).
    BASE_VEL common cruise velocity (default [0.5, 0, 0]).

OUTPUTS  ->  hetero_noise_moving/
    raw/sigma_*.csv               per-sigma HIL trajectory (6000 x 12)
    metrics.csv                   per-sigma ||Mx(T)||, tail_mean, settling time,
                                  endpoint distance to the sigma=0 baseline
...
PICK   = {"weak": 0.15, "medium": 0.30, "strong": 0.50}
