...
with a common cruise velocity AND a per-edge Kalman filter on the noisy
local-frame relative-position measurements.

Setup matches experiment_hetero_noise.py (moving variant) — same node order,
same control law, same common cruise base_vel = [0.5, 0, 0], same sigma sweep,
same baseline (HIL sigma=0). The only difference: every follower runs a small
3D Kalman filter on each of its neighbor measurements before feeding them to
the formation control law.

KF design (per (follower, neighbor) pair, independent):
  state     x ∈ ℝ³  : the relative position of neighbor j in follower i's body frame
  process   x_{k+1} = x_k + w_k,           Q = q · I  (random walk, q is fixed)
  measurement z_k = x_k + v_k,             R = σ² · I (per-axis Gaussian)
  init      x_0 = first measurement,        P_0 = R + Q
  predict   P_pred = P + Q
  gain      K = P_pred / (P_pred + R)
  update    x_post = x_pred + K (z - x_pred);  P_post = (1 - K) P_pred

Outputs go to hetero_noise_moving_kf/.
"""
